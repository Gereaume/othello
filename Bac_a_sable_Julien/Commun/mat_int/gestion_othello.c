...
**/
int retourner(int mat[N][N], int couleur, int lig, int col, int tab[54]){
...
int coup_possible(int mat[N][N], int couleur, int lig, int col){
	...
	int nb_pions_ret;
	copie(mat, mat2);
	int tab[54];											/** copie contenu de mat dans mat2	**/

	if((mat[lig][col] == 0) && !hors_mat(lig,col)){			/** si les coordonnées correspondent à une zone vide **/
		
		nb_pions_ret = retourner(mat2, couleur, lig, col, tab);		
