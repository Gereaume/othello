...
int lancer_jeu1(SDL_Renderer* renderer, SDL_Window* fenetre){
	...
	police = TTF_OpenFont("objet/Roboto-Black.ttf", 35); /** police de taille 30  **/


	SDL_Color c_noir = {0, 0, 0};
    SDL_Surface *retour = NULL, *r = NULL;  /** création de variables surfaces pour afficher du texte ou des 	images **/
    SDL_Texture *retourr, *rectangle;  /** creation de textures pour affichages du texte **/
	...
	retour = TTF_RenderText_Solid(police,"L'OTHELLO", c_noir);
	...
	if(fenetre)
	{
		...
		r = IMG_Load("objet/rect.bmp");

		rectangle = SDL_CreateTextureFromSurface(renderer,r);
		SDL_FreeSurface(r);
        ...
        while(cont){
			...
			SDL_QueryTexture(rectangle, NULL, NULL, &rect.w, &rect.h);
			...
			posx=event.motion.x;
                        ...
                        if(i%2 !=0){
                            if(aff_cerclenoir(mat,joueur1,joueur2,posx,posy,renderer,n, fenetre)==1 || aff_cerclenoir(mat,joueur1,joueur2,posx,posy,renderer,n, fenetre)==-2){
                                i++;
                            }
                            else if(aff_cerclenoir(mat,joueur1,joueur2,posx,posy,renderer,n, fenetre)==-1){ /** si on ne peut plus jouer on quitte 	**/
                                sleep(5);
                                cont = 0;
                                break;
                            }
                            ...
                            else if(aff_cercleblanc(mat,joueur1,joueur2,posx,posy,renderer,b, fenetre)==-1){
                                sleep(5);
				cont = 0;
                                break;
                            }
...
int lancer_jeu2(SDL_Renderer* renderer, SDL_Window* fenetre, int nb_coup_prevu){
	...
	SDL_FreeSurface(r);
		...
		SDL_Event event; /** création variable pour les évènements de la souris	**/
            	while(SDL_PollEvent(&event)){  /** tant qu'on à un évènement de la souris 	**/
                	switch(event.type){
		    	case SDL_QUIT: cont = 0;
				   break;
				...
				posx=event.motion.x;
			    	...
			    	else if(aff_cerclenoir(mat,joueur1,joueur2,posx,posy,renderer,n, fenetre)==-2){
                                	i++;
                           	}
                            	else if(aff_cerclenoir(mat,joueur1,joueur2,posx,posy,renderer,n, fenetre)==-1){ /** si on ne peut plus jouer on quitte 	**/
                                	sleep(5);
		...
		SDL_QueryTexture(rectangle, NULL, NULL, &rect.w, &rect.h);
    	    	SDL_RenderCopy(renderer,rectangle,NULL,&rect);
                if(aff_ordi(mat,joueur1,joueur2,renderer,b, fenetre,nb_coup_prevu, j)==1){
                	i++;
                }
                else if(aff_ordi(mat,joueur1,joueur2,renderer,b, fenetre,nb_coup_prevu, j)==-1){
                	sleep(15);
			cont = 0;
                        break;
                }
...
int aff_menu(SDL_Renderer* renderer, SDL_Window* fenetre, int j){
	...
	if(j==0){
		...
		surface = SDL_LoadBMP("objet/image.bmp"); /** telechargement icone de l'othello   **/
