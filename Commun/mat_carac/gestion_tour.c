...
void gestion_tour(char mat[N][N], t_joueur joueur1, t_joueur joueur2){
	...
	while(i<(N*N)-4){	
		...
		if(peut_jouer(mat, NOIR) == 1){
			...
			while(coup_possible(mat,NOIR,lig-'0',col-'0') == 0 || (!isdigit(lig)||!isdigit(col))){	/** Tant que le coup est impossible le joueur doit ressaisir des coordonnées correctes **/
		...
		if(peut_jouer(mat, BLANC) == 1){
			...
			while(coup_possible(mat,BLANC,lig-'0',col-'0') == 0 || (!isdigit(lig)||!isdigit(col))){	/** Tant que le coup est impossible le joueur doit ressaisir des coordonnées correctes **/
...
void gestion_tour_iaVSia(char mat[N][N], t_joueur joueur1, t_joueur joueur2, int nb_coup_prevu1, int nb_coup_prevu2){
	...
	while(i<(N*N)-4){	
		...
		if(peut_jouer(mat, NOIR) == 1){
			...
			if(nb_coup_prevu1 > (N*N-4-i)){
				tour_ordi(mat, NOIR, (N*N-4-i), &x, &y);
			}
			else{
				tour_ordi(mat, NOIR, (nb_coup_prevu1), &x, &y);
			}
			
			printf("\n\t%s joue à la ligne %i et colonne %i\n", joueur1.nom_joueur, x, y);
			ecrire_mat(mat,NOIR,x,y);					/** Une fois que l'ordinateur sait où il joue, on l'ecrit dans la matrice	**/
			nb_ret=retourner(mat,NOIR,x,y);
			joueur1.score += (nb_ret+1);
			joueur2.score -= nb_ret;
			printf("\n\tLe nb de pion(s) retourné(s) est de %i\n",nb_ret);
			i+=1;										/** On incrémente i	**/
		...
		if(peut_jouer(mat, BLANC) == 1){
			...
			if(nb_coup_prevu2 > (N*N-4-i)){
				tour_ordi(mat, BLANC, (N*N-4-i), &x, &y);
			}
			else{
				tour_ordi(mat, BLANC, (nb_coup_prevu2), &x, &y);
			}
